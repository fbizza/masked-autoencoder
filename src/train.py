...
if __name__ == '__main__':
    ...
    for e in range(args.total_epoch):
        ...
        print(f'Train loss on masked pixels is {avg_loss}.')
        ...
        val_losses = []
        with torch.no_grad():
            for val_img, _ in torch.utils.data.DataLoader(val_dataset, load_batch_size, num_workers=2):
                val_img = val_img.to(device)
                predicted_val_img, mask = model(val_img)
        ...
        print(f'Val loss on masked pixels is {avg_val_loss}.')
